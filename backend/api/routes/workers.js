...
router.post('/', auth, async (req, res) => {
  const { name, isRegistered, registrationDate, childrenCount, active } = req.body;
  try {
    const newWorker = new Worker({ name, isRegistered, registrationDate, childrenCount, active });
...
router.put('/:id', auth, async (req, res) => {
  const { name, isRegistered, registrationDate, childrenCount, active } = req.body;
  const workerFields = { name, isRegistered, registrationDate, childrenCount, active };
  try {
