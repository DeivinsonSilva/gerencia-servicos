...
onMounted(async () => {
  try {
    ...
    if (error.response && error.response.status === 403) {
      userCount.value = '-'; // Indica acesso negado para usuários
      // Tenta buscar os outros dados que não precisam de permissão de admin
      try {
        const [servicesRes, farmsRes] = await Promise.all([
          api.get('/services'),
          api.get('/farms'),
        ]);
        serviceCount.value = servicesRes.data.length;
        ...
        console.error('Erro ao buscar dados do dashboard (operador):', e);
      }
    } else {
