        ...
        <div class="card p-6">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 text-center">
            <div class="bg-slate-900/50 p-4 rounded-lg">
              <p class="text-sm text-slate-400">Serviços Cadastrados</p>
              <p class="text-3xl font-bold text-white mt-1">{{ serviceCount }}</p>
            </div>
            <div class="bg-slate-900/50 p-4 rounded-lg">
              <p class="text-sm text-slate-400">Fazendas Cadastradas</p>
              <p class="text-3xl font-bold text-white mt-1">{{ farmCount }}</p>
            </div>
            <div class="bg-slate-900/50 p-4 rounded-lg">
              <p class="text-sm text-slate-400">Usuários Registrados</p>
...
// Variáveis reativas para guardar as contagens
const serviceCount = ref(0);
const farmCount = ref(0);
const userCount = ref(0);

// onMounted é um "gancho" do Vue que roda o código assim que o componente é exibido na tela
onMounted(async () => {
  // Pega o token do localStorage
  const token = localStorage.getItem('authToken');
  if (!token) {
    console.error('Nenhum token encontrado, não é possível buscar dados.');
    return;
  }

  // Configuração do cabeçalho de autorização para todas as requisições axios
  const config = {
    headers: {
      'x-auth-token': token
    }
  ...
  try {
    // Fazemos as 3 chamadas à API em paralelo para mais performance
    const [servicesRes, farmsRes, usersRes] = await Promise.all([
      axios.get('http://localhost:3000/api/services', config),
      axios.get('http://localhost:3000/api/farms', config),
      axios.get('http://localhost:3000/api/users', config)
    ]);

    // Atualizamos nossas variáveis com o tamanho das listas recebidas
    ...
    // Se um usuário Operador tentar buscar os usuários, a API retornará um erro 403,
    // que será capturado aqui. Você pode tratar isso mostrando uma mensagem específica.
    if (error.response && error.response.status === 403) {
        // Como o Operador não pode ver usuários, vamos buscar apenas as outras duas contagens
        userCount.value = '-'; // Indica acesso negado
    } else {
